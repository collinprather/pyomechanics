...
#  compute the metrics for each joint angle, for each session_swing individually
metrics = []
for i, col in enumerate([col for col in df_source.columns if col not in ["session_swing", "time"]]):
    print(i)
    metric = {}
    for metric_fn in [root_mean_squared_error, median_absolute_error]:
        for session_swing in list(session_swings):
            df_joined_session_swing = df_joined.loc[df_joined["session_swing"] == session_swing]
            metrics.append({
                "session_swing": session_swing,
                "col": col,
                "metric": metric_fn.__name__,
                "value": metric_fn(df_joined_session_swing[f"{col}_target"], df_joined_session_swing[f"{col}_source"])
            })
pd.DataFrame.from_records(metrics).to_csv(f"./data/results.csv", index=False)
